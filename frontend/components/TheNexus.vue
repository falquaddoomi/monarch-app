...
import { Point2d, Point3d } from "@/utils/math";
import {
  useEventListener,
  useResizeObserver,
  useIntervalFn,
...
function rotate(event: MouseEvent | TouchEvent) {
  ...
  rxTarget = (0.5 - y / innerHeight) * 90;
  ryTarget = -(0.5 - x / innerWidth) * 90;
}
...
function pulse() {
  for (const entity of [...dots, ...links]) {
    ...
    /** set timers */
    setTimeout(() => (entity.colorTarget = [...pulseColor]), start);
    setTimeout(() => (entity.colorTarget = [...baseColor]), reset);
  }
}

...
<template>
  ...
  <AppSection>
    ...
    <!-- status -->
    <AppStatus v-if="pending" code="loading">Loading posts</AppStatus>
    <AppStatus v-if="error" code="error">Error loading posts</AppStatus>

    <!-- list of posts -->
    <AppFlex v-if="blogPosts && blogPosts.length" direction="col" gap="big">
      <AppPost
...
const {
  ...
  error,
  refresh,
} = await useLazyAsyncData(getBlogPosts);
